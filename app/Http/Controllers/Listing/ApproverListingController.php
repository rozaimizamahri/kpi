...
class ApproverListingController extends Controller
{
    public function approver(Request $request, $appId){
    ...
    public function declare(Request $request){
        ...
        // Find All CKPI Admin
            $pool_users = Users::whereIn('approver_access',['1','2','3'])->get(); 
            $i                      = 0; 
            $staffname              = ''; 
            $email_address          = ''; 
        ...
        // Find All Assignees
            $pool_assignees = Ckpi_assignees::select( 
                                            'app_id', 
                                            'assignee_email'  
                )
                ->groupBy('app_id','assignee_email') 
                ->where('app_id','=',$app_id) 
                ->where('is_active','=','Y')
                ->get();  
            $i                          = 0;  
            ...
            foreach ($pool_assignees as $pool_assignee) { 
                if($i==0){    
                    $assignee_email    = ''.$insert_text.''.$pool_assignee->assignee_email; 
                }
                else{    
                    $assignee_email    = $assignee_email.','.$insert_text.''.$pool_assignee->assignee_email; 
                }
                $i++;
            } 
        // Find All Assignees  

        $result = EmailController::approverDeclare($staff_name,$semail,$staffname,$email_address,$kpi_owner_email,$assignee_email,$app_name);
        if(!$result) {   
            return back()->with('email_failed', 'SMTP email failed.');           
        } else {

            // Clear all Quarter to 'N' first
            ...
            // Set Y to Approver for Current Quarter 
                $assignees_new = Ckpi_assignees::where('app_id',$app_id)
                    ->where('quarter_id',$quarter_id)
                    ->update(
                        [
                            'approver'             => 'Y'
            ...
            // Activate next quarter
                $assignees2 = Ckpi_assignees::where('app_id',$app_id) 
                ->where('quarter_id','=',$count_plus_1) 
                ->update(
                    [ 
                        'is_active'              => 'Y'
                    ]
            ...
            $applications = Ckpi_apps::where('id',$app_id)
                ->update(
                    [
                        ...
                        'app_stage'             => '12',
                        'q'.$quarter_id.''      => 'Y',
                        'q'.$quarter_id.'_final'=> 'Y',

                        'app_approved_is'       => 'Y',
                        'app_approved_by_name'  => $staff_name,
                        'app_approved_by_email' => $semail2,
                        'app_approved_by_date'  => Carbon::Now(),
                        'app_approved_remark'   => $remark 
                    ]
                ); 

            return redirect('/home')->with('approver_declare', 'Corporate KPI successfully approved.');
    ...
    public function rework(Request $request){
        ...
        $result = EmailController::approverRework($staff_name,$semail,$staffname,$email_address,$app_name);
        if(!$result) {   
            return back()->with('email_failed', 'SMTP email failed.');           
        } else {
            ...
            return redirect('/home')->with('approver_rework', 'Request to rework has been submitted.');
