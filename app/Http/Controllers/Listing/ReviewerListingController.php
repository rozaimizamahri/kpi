...
class ReviewerListingController extends Controller
{
    public function reviewer(Request $request, $appId){
    ...
    public function declare(Request $request){
        ...
        // Find Checker CKPI Admin
            $pool_users = Users::whereIn('approver_access',['3'])
                ->get();
               // ->toArray();  
            $i                      = 0; 
            $staffname              = ''; 
        ...
        // Find Checker CKPI Admin
            $pool_checkers = Users::whereIn('approver_access',['1'])
                ->get();
               // ->toArray();  
            $i                      = 0; 
            $checker_name              = ''; 
            ...
            foreach ($pool_checkers as $pool_checker) { 
                if($i==0){   
                    $checker_name      = $pool_checker->staff_name;   
                    $checker_email  = ''.$insert_text.''.$pool_checker->email_address;   
                }
                else{   
                    $checker_name      = $checker_name.','.$pool_checker->staff_name;  
                    $checker_email  = $checker_email.','.$insert_text.''.$pool_checker->email_address;  
                }
                $i++;
            } 
        // Find Checker CKPI Admin

        $result = EmailController::reviewerDeclare($staff_name, $semail, $staffname, $email_address, $checker_name, $checker_email, $app_name);
        if(!$result) {   
            return back()->with('email_failed', 'Declare Failed. SMTP email failed.');           
        } else {
            
            $applications = Ckpi_apps::where('id',$app_id)
                ->update(
                    [
                        'app_status'            => 'FINALAPPROVAL', 
                        'app_stage'             => '11',
                        
                        'app_reviewed_is'       => 'Y',
                        'app_reviewed_by_name'  => $staff_name,
                        'app_reviewed_by_email' => $semail2,
                        'app_reviewed_by_date'  => Carbon::Now(),
                        'app_reviewed_remark'   => $remark  
                    ]
                ); 

            return redirect('/home')->with('reviewer_declare', 'KPI successfully approved and submitted to final approver for review.');
    ...
    public function rework(Request $request){
        ...
        $result = EmailController::reviewerRework($staff_name,$semail,$staffname,$email_address,$app_name);
        if(!$result) {   
            return back()->with('email_failed', 'Rework Failed : SMTP email failed.');           
        } else {
            
            $applications = Ckpi_apps::where('id',$app_id)
                ->update(
                    [
                        'app_status'            => 'REWORKCHECK', 
                        'app_stage'             => '9',  
            ...
            return redirect('/home')->with('reviewer_rework', 'Request to rework has been submitted.');
